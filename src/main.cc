...
int main(int argc, char* argv[])
{
	...
	std::vector<float> bone_line_vertices;
	...
	create_skeleton(bone_vertices, bone_lines, mesh);
	create_cylinders(cyl_vertices, cyl_lines, mesh);
	...
	// bone_pass_input.assign_index(bone_line_vertices.data(), bone_line_vertices.size(), 2);
	bone_pass_input.assign_index(bone_lines.data(), bone_lines.size(), 2);
	RenderPass bone_pass(-1,
			bone_pass_input,
			{ skeleton_vertex_shader, skeleton_geometry_shader, skeleton_fragment_shader},
			{ std_model, std_view, std_proj, std_light },
			{ "fragment_color" }
			);

	RenderDataInput cyl_pass_input;
	bone_pass_input.assign(0, "vertex_position", cyl_vertices.data(), cyl_vertices.size(), 4, GL_FLOAT);
	// bone_pass_input.assign_index(bone_line_vertices.data(), bone_line_vertices.size(), 2);
	bone_pass_input.assign_index(cyl_lines.data(), cyl_lines.size(), 2);
	RenderPass cyl_pass(-1,
			cyl_pass_input,
			{ skeleton_vertex_shader, skeleton_geometry_shader, cylinder_fragment_shader},
			{ std_model, std_view, std_proj, std_light },
			{ "fragment_color" }
			);
	...
	while (!glfwWindowShouldClose(window)) {
		...
		// draw_cylinder = gui.isTransparent();
